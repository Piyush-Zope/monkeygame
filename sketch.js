...
function setup(){
 createCanvas(600,300);
  ...
  grass.addImage("grass.png",grassImg)
  grass.scale=1 
...
function draw(){
  ...
  if (gameState === PLAY){
     text("TIME Survived="+score, 400, 20);
  text("BANANAS Eaten="+bananaScore,100,20);
    obstacles();
    bananas();
  ...
  if (gameState === END){
    ...
    if (keyDown("r")){
     ...
     if (frameCount%9 === 0){
    rock= createSprite(620,300,50, 50 )
    rock.addAnimation("rock",rockImg);
    rock.scale = 0.080;
    rock.velocityX =-(4+score*1.5/100);           
    rock.lifetime = 220;
     }
